...
namespace Integrating 
{
    class Calculation 
    {
        ...
        public Calculation(Func<double, double> f)   
        ...
        public double Monte_Carlo(double left, double right, int throwsMultiplier, double sup , double inf ,int strats_count) {
            ...
            void calculating(int part) 
            {
                ...
                {   //Console.WriteLine("[Thread №{0}] Страта - [{1}; {2}]x[{3}, {4}]", part, x_id, x_id + dt, y_id, y_id + dt);
                    ...
                    if (i % 117 == 0) EventDraw?.Invoke(x, y, part); //Проверяем вхождение точки в область под графиком
                    if (function(x) >= 0) 
                    {
                    ...
                    {
                        if (y >= function(x) && y < 0) count--; //Console.WriteLine("[{2}] ({1}) Y[{5}, {6}]", x, y, part, x_id, x_id + dx, y_id, y_id + dy);
